...
原始审计发现仍有保留价值，但不能再整体视为当前缺陷清单。006～009 已完成 clean switch、
ReaderShell 的 skip link 和目录目标尺寸，以及可搜索、虚拟化、双栏联动的出版工作台。
当前剩余问题集中在**正文标题视觉层级、图书详情、页面级样式重复和匿名管理增强探测**。

优先级建议：

1. **P0：修复明确的阅读与 CSS 缺陷。** 恢复正文 `h1`–`h4` 可见层级，并修复两处无效的
   `var(--color-white)-space` 属性。
2. **P1：收口图书详情。** 移动详情改为全屏覆盖，复用封面/占位封面，并把最多 200 项的
   数据投影整理为可浏览而有界的目录预览。
3. **P2：收束剩余壳层。** 完成导入页的管理导航层级，逐步减少页面级重复样式，并为匿名
...
| UI-004 | 正文 `h1`–`h4` 没有可见层级         | 未解决   | `reader.css` 仍只给 `.reader-document` 统一正文样式                                   | MEDIUM |
| UI-005 | 移动详情不是全屏覆盖                | 未解决   | 详情仍使用有视口边距、圆角和居中布局的 dialog                                         | HIGH   |
| UI-006 | 详情忽略封面及占位封面              | 未解决   | `BookDetails` 仍未读取 `coverUrl`                                                     | MEDIUM |
| UI-007 | 详情直接渲染最多 200 项目录         | 未解决   | presentation 上限仍直接成为组件渲染列表                                               | MEDIUM |
| UI-008 | 管理预览是超长全书表单              | 已解决   | 当前为虚拟化结构列表、单项编辑、sticky ReaderShell 预览和移动模式切换                 | -      |
| UI-009 | 导入暴露 ID 且不展示限制            | 部分解决 | 已有书名选择器、限制、真实进度和取消；共享管理导航层级仍较弱                          | LOW    |
| UI-010 | 页面重复壳层和样式                  | 部分解决 | 出版工作台已有共享 primitives；首页、登录、书库、详情和部分管理页仍保留大量页面级样式 | MEDIUM |
| UI-011 | 两处无效 `var(--color-white)-space` | 未解决   | 详情描述和 Passkey `.sr-only` 中仍存在                                                | MEDIUM |
| UI-012 | 匿名书库请求管理 API                | 未解决   | `AdminLibraryEnhancement` 仍会在所有客户端加载后请求 `/api/manage/library`            | LOW    |
