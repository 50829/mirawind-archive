...
4. **Generate tasks.md**: Read the tasks template from TASKS_TEMPLATE (from the JSON output above) and use it as structure. If TASKS_TEMPLATE is empty, fall back to `.specify/templates/tasks-template.md`. Fill with:
   ...
   - Each phase includes: story goal, independent test criteria, constitution-required
     evidence tests, any additional tests requested by the spec, and implementation tasks
...
2. **From Contracts**:
   ...
   - If the contract touches a constitution-critical path, or tests are otherwise requested:
...
- **Phase 3+**: User Stories in priority order (P1, P2, P3...)
  - Within each story: Required evidence tests → Models → Services → Endpoints → Integration
