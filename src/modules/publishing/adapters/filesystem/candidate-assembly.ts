...
export async function assembleCandidate(
  ...
  try {
    ...
    if (sha256(documentJson) !== input.documentSha256)
      throw new Error("CANDIDATE_DOCUMENT_INTEGRITY_MISMATCH");
    ...
    if (
      ...
      throw new Error("CANDIDATE_ORIGINAL_CAPTURE_INVALID");
    for (const original of originalRecord.files) {
      input.signal?.throwIfAborted();
      const relativePath = String(original.path);
      if (relativePath !== "originals/" + original.id)
        throw new Error("CANDIDATE_ORIGINAL_PATH_INVALID");
      ...
      if (
        ...
        throw new Error("CANDIDATE_ORIGINAL_INVALID");
      const destination = resolve(versionDirectory, relativePath);
      ...
      for await (const chunk of createReadStream(destination)) {
        ...
        if (size > Number(original.size))
          throw new Error("CANDIDATE_ORIGINAL_CHANGED");
        hash.update(chunk);
      }
      const digest = hash.digest("hex");
      if (size !== original.size || digest !== original.sha256)
        throw new Error("CANDIDATE_ORIGINAL_CHANGED");
      files.record({ path: relativePath, sha256: digest, size });
    }
    ...
    for (const resource of book.resources) {
      ...
      if (
        ...
        throw new Error("CANDIDATE_RESOURCE_INVALID");
      ...
      if (
        ...
        throw new Error("CANDIDATE_RESOURCE_INTEGRITY_MISMATCH");
      const image = await inspectRasterImage({
        bytes,
        filename: resource.path,
      });
      await files.write(resource.path, bytes);
