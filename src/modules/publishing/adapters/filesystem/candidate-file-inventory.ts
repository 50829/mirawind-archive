...
export interface CandidateFileSink {
  record(file: CandidateFileDescriptor): void;
...
function validateDescriptor(file: CandidateFileDescriptor): void {
  if (
    file.path.length === 0 ||
    file.path === "." ||
    file.path === ".." ||
...
export function createCandidateFileInventory(
  root: string,
): CandidateFileInventory {
  const candidateRoot = resolve(root);
  const files = new Map<string, CandidateFileDescriptor>();

  const record = (file: CandidateFileDescriptor): void => {
