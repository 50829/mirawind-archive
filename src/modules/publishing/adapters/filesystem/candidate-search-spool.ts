...
function record(value: unknown): Readonly<Record<string, unknown>> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("CANDIDATE_SEARCH_SPOOL_INVALID");
  }
  return value as Readonly<Record<string, unknown>>;
}

export async function readCandidateSearchSpool(
  path: string,
): Promise<SearchSpool> {
  const metadata = await stat(path);
  if (!metadata.isFile() || metadata.size > maximumCandidateSpoolBytes) {
    throw new Error("CANDIDATE_SEARCH_SPOOL_LIMIT");
  }
