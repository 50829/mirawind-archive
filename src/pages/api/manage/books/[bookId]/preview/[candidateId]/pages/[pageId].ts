...
export const GET: APIRoute = async ({ locals, params }) => {
  ...
  if (!bookId || !isOpaqueId("draftCandidate", candidateId) || !pageId) {
  ...
  const candidate = publishing.findPreviewCandidate(candidateId, bookId);
  if (
    !session ||
    !book?.draftImportId ||
    candidate?.attemptId !== candidateId ||
